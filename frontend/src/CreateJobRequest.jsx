...
export default function CreateJobRequest({ user, onBack }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setServerError("Could not reach the server. Make sure Django is running.");
    }
    setLoading(false);
  };

  const handleReset = () => {
    setTitle(""); setDescription(""); setCategory(""); setRate("");
    setLocation(""); setDate(""); setErrors({});
    setSubmitted(false); setServerError(null);
  };
        ...
        {submitted ? (
          <div style={{
            ...
            <p style={{ color: "#64748b", fontSize: 14, margin: "0 0 24px", lineHeight: 1.6 }}>
              Your request for <strong style={{ color: "#e2e8f0" }}>"{title}"</strong> has been saved to the database and is now visible on the Job Board.
            </p>
