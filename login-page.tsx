...
const Dashboard = ({ user, onLogout }) => (
  <div style={{ textAlign: "center", padding: 20 }}>
    <div style={{
      width: 72, height: 72, borderRadius: "50%", background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.accent})`,
      display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 32, color: "#fff", fontWeight: 800
    }}>
      {user.name[0].toUpperCase()}
    </div>
    <h2 style={{ margin: "0 0 4px", color: COLORS.text }}>Welcome, {user.name}!</h2>
    <span style={{
      display: "inline-block", padding: "4px 14px", borderRadius: 20, fontSize: 13, fontWeight: 700,
      background: user.role === "customer" ? COLORS.primaryLight : "#D1FAE5",
      color: user.role === "customer" ? COLORS.primary : "#059669", marginBottom: 24
    }}>
      {user.role === "customer" ? "🏠 Customer" : "🔧 Worker"}
    </span>
    <div style={{ background: "#F1F5F9", borderRadius: 12, padding: 20, marginBottom: 24, textAlign: "left" }}>
      <p style={{ margin: "0 0 8px", fontSize: 14, color: COLORS.textMuted }}><strong style={{ color: COLORS.text }}>Email:</strong> {user.email}</p>
      <p style={{ margin: 0, fontSize: 14, color: COLORS.textMuted }}><strong style={{ color: COLORS.text }}>Session Token:</strong> <code style={{ fontSize: 11, background: "#E2E8F0", padding: "2px 6px", borderRadius: 4 }}>{user.token}</code></p>
    </div>
    <p style={{ fontSize: 13, color: COLORS.textMuted, marginBottom: 20 }}>
      This is a prototype dashboard. In production, {user.role === "customer" ? "you'd see nearby jobs and post requests here." : "you'd see available jobs and your earnings here."}
    </p>
...
export default function App() {
  ...
  const handleSignup = () => {
    ...
    setTimeout(() => {
      ...
      setUser({ ...newUser, token: genToken() });
