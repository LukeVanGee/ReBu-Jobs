...
const VerifyScreen = ({ email, code, onVerified, onBack }) => {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ width: 64, height: 64, borderRadius: "50%", background: COLORS.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 32 }}>✉️</div>
      ...
      <div style={{ background: "#FFFBEB", border: "1.5px dashed #F59E0B", borderRadius: 10, padding: "12px 16px", marginBottom: 24, textAlign: "left" }}>
        <p style={{ margin: "0 0 4px", fontSize: 12, fontWeight: 700, color: "#92400E" }}>📧 Simulated Email Preview</p>
      ...
      <div style={{ marginTop: 16 }}>
        <span onClick={onBack} style={{ fontSize: 13, color: COLORS.textMuted, cursor: "pointer", fontWeight: 600 }}>← Back to sign up</span>
      </div>
    </div>
  );
};

/* ─── Placeholder Homepage (replace with your ReBuHomepage import in your project) ─── */
const HomePlaceholder = ({ user, onLogout }) => (
  <div style={{ minHeight: "100vh", background: "#0a0f1a", color: "#e2e8f0", fontFamily: "'Inter', -apple-system, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
    ...
    <p style={{ color: "#64748b", fontSize: 13 }}>({user.role === "customer" ? "🏠 Customer" : "🔧 Worker"} · {user.email})</p>
